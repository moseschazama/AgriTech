...
<div class="section" style="background:var(--bg-2);">
  <div class="container">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px;">
      <div><h1 style="font-family:var(--font-display);font-size:1.5rem;font-weight:800;color:var(--text);">💡 My Innovations</h1>
    ...
    <div style="display:grid;gap:16px;">
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-lg);padding:22px;display:flex;gap:18px;align-items:flex-start;flex-wrap:wrap;">
          ...
          <div style="flex:1;min-width:200px;">
            <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px;">
              <span class="badge {{ $sc[$innovation->status]??'badge-gray' }}" style="font-size:.68rem;">{{ ucfirst($innovation->status) }}</span>
              <span class="badge badge-gray" style="font-size:.68rem;">{{ ucwords(str_replace('_',' ',$innovation->category)) }}</span>
              @if($innovation->in_competition)<span class="badge badge-earth" style="font-size:.68rem;">🏆 Competition</span>@endif
            </div>
            <div style="font-family:var(--font-display);font-size:.95rem;font-weight:700;color:var(--text);margin-bottom:4px;">{{ $innovation->title }}</div>
            <div style="font-size:.8rem;color:var(--text-muted);line-height:1.5;margin-bottom:10px;">{{ Str::limit($innovation->description,120) }}</div>
            <div style="display:flex;gap:16px;font-size:.78rem;color:var(--text-muted);flex-wrap:wrap;">
            ...
            @if($innovation->status==='rejected'&&$innovation->rejection_reason)
              <div style="margin-top:10px;background:#fef2f2;border:1px solid #fecaca;border-radius:var(--radius-md);padding:8px 12px;font-size:.78rem;color:#dc2626;"><i class="fas fa-info-circle"></i> {{ $innovation->rejection_reason }}</div>
            @endif
          </div>
          <div>
      ...
      @empty
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:var(--radius-xl);padding:60px;text-align:center;">
          ...
          <h3 style="font-family:var(--font-display);margin-bottom:8px;">No innovations yet</h3>
